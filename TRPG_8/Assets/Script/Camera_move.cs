...
public class Camera_move : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        try
        {
            if (playerAt == "A" && GameObject.Find("ME").transform.position.x + 3.5f > borderRight)//不能往右
            {
                gameObject.transform.position = new Vector3(borderRight - 3.5f, gameObject.transform.position.y, -10f);
                stopX = true;
            }
            if (playerAt == "A" && GameObject.Find("ME").transform.position.x - 3.5f < borderLeft)//不能往左
            {
                gameObject.transform.position = new Vector3(borderLeft + 3.5f, gameObject.transform.position.y, -10f);
                stopX = true;
            }
            if (playerAt == "A" && GameObject.Find("ME").transform.position.y + 2f > borderTop)//不能往上
            {
                gameObject.transform.position = new Vector3(gameObject.transform.position.x, borderTop - 2f, -10f);
                stopY = true;
            }
            if (playerAt == "A" && GameObject.Find("ME").transform.position.y - 2f < borderBottom)//不能往下
            {
